...
if {$tl_enabledcc} {
	...
	bind dcc o|o tlreset dcc:tl_reset
}
...
set command [lindex [split $arg] 0]
	...
	switch -exact -- [string tolower $command] {
		"" {
			...
			foreach chan [channels] {
			if [file exists topic.$chan] {
			set g [open "topic.$chan" r]
			set tl_lock([string tolower $chan]) 1
			gets $g line
			set tl_topic([string tolower $chan]) $line
			close $g
			putlog "Holding topic $tl_topic([string tolower $chan]) on $chan"
				}
			}
		}
		"tlock" {
...
proc dcc:tl_reset {hand idx arg} {
...
set chan [lindex [split $arg] 0]
	...
	if {![botisop $chan]} {
		putidx $idx "I'm not opped on $chan."
	} else {
		if {$tl_lock([string tolower $chan]) != 1} {
			putidx $idx "I am not holding the topic in $chan"
			} else {		
			putserv "TOPIC $chan :$tl_topic([string tolower $chan])"
			putlog "Reset topic to $tl_topic([string tolower $chan]) on $chan"
			}
		}
	}
}
...
global tl_lock tl_topic tl_notice tl_globflags tl_chanflags tl_nochans
	...
	if {($chan == "") || ($chan == "*")} {
		...
		if {![tl_botonchan $chan]} {
			...
			set g [open "topic.$chan" w]
			puts $g "$tl_topic([string tolower $chan])"
			close $g
			if {$tl_topic([string tolower $chan]) != [topic $chan]} {
				if {([botisop $chan]) || (![string match "*t*" [lindex [getchanmode $chan] 0]])} {
					putserv "TOPIC $chan :$tl_topic([string tolower $chan])"
...
global tl_ver tl_lock tl_topic
set command [lindex [split $arg] 0]
	...
	switch -exact -- [string tolower $command] {
		...
		"tlreset" {
			putidx $idx "###  tlreset \[chan\]"
			putidx $idx "   will reset the topic of the specified channel."
		}
		"default" { 
...
putlog "TCL loaded: topiclock.tcl v$tl_ver by Sup <temex@iki.fi> additions by Paladinz"

foreach chan [channels] {
global tl_lock tl_topic
if [file exists topic.$chan] {
	set g [open "topic.$chan" r]
	gets $g line
